...
urlpatterns = [
    ...
    path("errors/404/", views.preview_404, name="preview-404"),
    path("errors/500/", views.preview_500, name="preview-500"),
    ...
    path("invite/<str:token>/", views.invitation_view, name="invitation"),
    path(
    ...
    path(
        "comunity/<slug:slug>/",
        views.community_canvas,
        name="community-canvas",
    ),
    path(
        "comunity/<slug:slug>/leaders/",
        views.community_leaders,
        name="community-leaders",
    ),
    path(
        "comunity/<slug:slug>/details/",
        views.community_detail,
        name="community-detail",
    ),
    path(
        "comunity/<slug:slug>/update/",
        views.update_community_details,
        name="update-community-details",
    ),
    path(
        "comunity/<slug:slug>/members/<int:user_id>/remove/",
        views.remove_community_member,
        name="remove-community-member",
    ),
    path(
        "comunity/<slug:slug>/join-requests/<int:request_id>/approve/",
        views.approve_join_request,
        name="approve-join-request",
    ),
    path(
        "comunity/<slug:slug>/join-requests/<int:request_id>/decline/",
        views.decline_join_request,
        name="decline-join-request",
    ),
    path(
        "comunity/<slug:slug>/guide/",
        views.community_guide,
        name="community-guide",
    ),
    path(
        "comunity/<slug:slug>/leave/",
        views.leave_community,
        name="leave-community",
    ),
    path(
        "comunity/<slug:slug>/delete/",
